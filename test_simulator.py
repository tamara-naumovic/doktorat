...
def main():
    ...
    # student test
    opcije = OpcijeSimulacije(tabela_konfiguracije="testPodaci/tk2.csv", interval_stampanja= 0.1, interval_integracije= 0.1, duzina_simulacije= 10)
    print("----------------Niz blokova-------------------")
    sim.ucitaj_blokove(opcije)
    print(json.dumps(opcije.niz_blokova, default=print, cls=sim.DecimalEncoder))
    # pprint.pprint(opcije.niz_blokova)
    print("----------------Obradjen niz blokova-------------------")
    sim.obradi_niz_blokova(opcije)
    print(json.dumps(opcije.niz_obradjen, default=print, cls=sim.DecimalEncoder))
    ...
    print(json.dumps(opcije.niz_sortiran, default=print,cls=sim.DecimalEncoder))
    # print("----------------Niz izlaza-------------------")
    ...
    print(json.dumps(opcije.matrica_izlaza, indent=4,cls=sim.DecimalEncoder))
    ...
    with open(result_file_name, 'w') as fp:
        json.dump(opcije.matrica_izlaza, fp,cls=sim.DecimalEncoder)
    kraj = time.time()
